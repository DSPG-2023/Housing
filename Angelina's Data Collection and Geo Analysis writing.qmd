...
Mapping our house quality output using GIS enables us to visualize the AI-model output for vegetation, siding, gutters, and roof characteristics. This will allow us to visualize locations of houses in good conditions versus poor condition. Visualization is one way to analyze geographic data, but is not the only way. In the future, it could be beneficial to utilize statistical analysis techniques to understand the spatial relationship between data and visualize clusters.

#### Geocoding Addresses

To visualize addresses for the communities, first we needed to geocode (provide latitude and longitude for each address). There many types of software that can be used to geocode addresses.

Base code was from [storybench](https://www.storybench.org/geocode-csv-addresses-r/){target="_blank"}.
